...
s_extract_kmer_features(BamQuerySequence& query,
    ...
    float* features)
{
    constexpr int base_features = EvalKmerFeaturesGenerator::EVAL_BASE_FEATURES;
    const int features_per_kmer = kmer_size * base_features;
    ...
    if (query.fwd_qs[fwd_offset] == ctx.FWD_MOD_BASE_CODE) {
        ...
        offset = fwd_offset;
        strand = FWD;
	fn = 1.0 * kinetics.fn() / 100.0;
	rn = 1.0 * kinetics.rn() / 100.0;
    } else {
...
bool EvalKmerFeaturesGenerator::get_next_sample_features(float* features, int& offset, int& strand)
{
    if (M_sample_idx >= M_num_samples) return false;
    offset = M_sample_offsets[M_sample_idx++];
    s_extract_kmer_features(M_query, M_kinetics, M_ctx, M_kmer_size, offset, strand, features);
    return true;
}
